...
const getSumBySheetName = (data, uniqueDates, sheet) => {
    ...
    return uniqueDates.map((d) => {
        const summary = sheetData.filter(o => o.date == d).map(k => [sheet] ? parseInt(k[sheet]) : 0).reduce((acc, curr) => { return acc + curr })
        const parsedDate = stringToDate(d)
        // console.log(d, parsedDate.getDay(), parsedDate.getMonth(), parsedDate.getFullYear())
