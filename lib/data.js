...
const getSumBySheetName = (data, uniqueDates, sheet) => {
    ...
    return uniqueDates.map((d) => {
        const summary = sheetData.filter(o => o.date == d).map(k => parseInt(k[sheet])).reduce((acc, curr) => { return acc + curr })
        return { 'date': d, 'value': summary }
    })
...
const getStats = async (data, sheets) => {
    ...
    for (let i = 0; i < sheets.length; i++) {
        ...
        stats[sheet] = getSumBySheetName(data, uniqueDates, sheet)
    }
